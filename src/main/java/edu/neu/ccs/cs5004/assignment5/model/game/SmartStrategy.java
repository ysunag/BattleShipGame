package edu.neu.ccs.cs5004.assignment5.model.game;


import java.util.Random;

public class SmartStrategy extends AbstractStrategy {
  /**
   * Method hashCode().
   *
   * @return return the hashCode of an object.
   */
  @Override
  public int hashCode() {
    return 33;
  }

  /**
   * The method equals.
   *
   * @param obj another object.
   * @return if two objects are same type, return true, else false.
   */

  @Override
  public boolean equals(Object obj) {
    if (this == obj) {
      return true;
    }
    if (obj == null || getClass() != obj.getClass()) {
      return false;
    }
    return true;
  }

  /**
   * smartly generate an attack position.
   * @param map the map used to attack.
   * @return a position.
   */
  public Position generateAttackPosn(MapI map) {
    Position posn = null;

    boolean valid = false;
    while (!valid) {
      Random random = new Random();
      Integer row = 1 + random.nextInt(Map.ROWNUMBER);
      Column col = Column.find(random.nextInt(Map.COLUMNNUMBER));
      posn = new Position(row, col);
      if (map.getCell(posn).getWasHit()
              ||
              (col.getCol() + 1 < Map.COLUMNNUMBER
                      &&
                      map.getCell(new Position(row, Column.find(col.getCol() + 1))).getWasHit())
              ||
              (col.getCol() - 1 >= 0
                      &&
                      map.getCell(new Position(row, Column.find(col.getCol() - 1))).getWasHit())
              ||
              (row - 1 >= 1
                      &&
                      map.getCell(new Position(row - 1, col)).getWasHit())
              ||
              (row + 1 <= Map.ROWNUMBER
                      &&
                      map.getCell(new Position(row + 1, col)).getWasHit())) {
        continue;
      } else {
        valid = true;
      }
    }

    return posn;
  }
}
